...
- `confirm.md` — 針對 `latest.md` 前 30 名，額外抓 FinMind 季報算出的存貨周轉天期、應收帳款收現天期，並標註跟上一季比是加快/轉慢，可以直接在 GitHub 上點開瀏覽。
- `confirm.csv` — 同一份確認結果的完整版本。金融股等沒有存貨概念的公司會顯示空白，不是資料錯誤（見 README 主頁「季報二次確認」）。
